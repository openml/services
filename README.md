...
-   Linux or MacOS (Intel or Apple Silicon)
    -   **Apple Silicon (M1/M2/M3/M4)**: See [macOS Apple Silicon Setup Guide](MACOS_M4_SETUP.md) for special instructions
    -   Docker Desktop with Rosetta 2 emulation is required for Apple Silicon
...
-   _Frontend_: localhost:8000
-   _Database_: localhost:3306, filled with test data.
-   _ElasticSearch_: localhost:9200 or localhost:8000/es, filled with test data.
-   _Rest API_: localhost:8080
-   _Minio_: console at localhost:9001, filled with test data.
